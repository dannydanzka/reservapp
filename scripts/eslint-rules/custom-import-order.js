/**
 * Custom import order rule - "Member-First Import Sorting" - Adapted for React Native
 *
 ...
 * - Added React Native specific module detection
 * - Added support for @components, @screens, @hooks, @libs paths
 * - Added .native.ts/.ios.ts/.android.ts file support
 *
 * Within each group:
...
/**
 * React Native specific modules that should be treated as external
 */
...
/**
 * Check if a module is a React Native specific module
 * @param {string} name - Module name
 * @returns {boolean}
 */
function isReactNativeModule(name) {
  return REACT_NATIVE_MODULES.some(rnModule => name.startsWith(rnModule));
}
...
function getImportGroup(source, pathGroups = [], excludedImportTypes = []) {
  ...
  if (source === 'react') return 'builtin'; // React is special case
  if (isReactNativeModule(source)) return 'external';
  ...
  if (source.startsWith('@/') || source.startsWith('@components') || 
      source.startsWith('@screens') || source.startsWith('@hooks') || 
      source.startsWith('@libs') || source.startsWith('@store') ||
      source.startsWith('@utils') || source.startsWith('@types') ||
      source.startsWith('@ui')) return 'internal';
...
/**
 * Custom import order rule - "Member-First Import Sorting" for React Native
 *
 * Sorts imports by the imported member name (not the module path):
 * - Destructured/named imports come first: import { StyleSheet } from 'react-native'
 ...
 *
 * Adapted for React Native with proper path group handling
 */
const customImportOrderRule = {
  ...
  meta: {
    docs: {
      ...
      description:
        'Enforce member-first import sorting: orders by imported member name with destructured imports before namespace imports before default imports (React Native adapted)',
      recommended: false,
    },
    fixable: 'code',
    schema: [
