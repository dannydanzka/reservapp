...
import { Modal, Pressable, Animated, Dimensions, BackHandler } from 'react-native';
...
export const ModalProvider: React.FC<ModalProviderProps> = ({ children }) => {
  ...
  const update = (id: string, content: ReactNode, options?: ModalOptions) => {
    setModals(prev => prev.map(modal => 
      modal.id === id 
        ? { ...modal, content, options: { ...modal.options, ...options } }
        : modal
    ));
  };

  return (
    <ModalContext.Provider value={{ show, hide, hideAll, update }}>
      {children}
...
export const AlertModal: React.FC<{
  ...
  const getIcon = () => {
    ...
    switch (type) {
      case 'warning': return <X {...iconProps} color={theme.colors.warning[500]} />;
      case 'error': return <X {...iconProps} color={theme.colors.error[500]} />;
      case 'success': return <X {...iconProps} color={theme.colors.success[500]} />;
      default: return <X {...iconProps} color={theme.colors.info[500]} />;
    }
  };
...
const AlertButton = styled.TouchableOpacity<{ variant: 'primary' | 'secondary' }>`
  ...
  border-color: ${props => props.variant === 'secondary' ? theme.colors.gray[300] : 'transparent'};
`;
